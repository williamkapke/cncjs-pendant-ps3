...
module.exports = function(options, callback) {
	...
	function connectPendant () {
	    ...
	    */
		...
		// Z Probe - Should be the same gcode generated by the widget in the ui
		controller.on('square:press', function(data) {
			if (r1) {
				...
				if(useTLO) {
					socket.emit('command', options.port, 'gcode', 'G4 P1');
					// Set tool length offset
					'G43.1 Z[posz-'+ touchPlateHeight +']'
				}
				else {
					// Set the active WCS Z0
					socket.emit('command', options.port, 'gcode', 'G10 L' + touchPlateHeight + ' P1 Z' + touchPlateHeight);
				}
